...
// ~4 km bbox padding - operators whose bboxes nearly touch count as adjacent.
const adjacencyPaddingDegrees = 0.04;
// Evenly distributes hues - each new hue lands far from existing ones on the
// color wheel, so adjacent operators get visibly different colors by default.
const goldenAngle = 137.508;

/**
 * Greedy graph coloring - adjacent operators get distinct golden-angle-spaced
 * hues so neighboring polygons don't blend. Deterministic for a given feature
 * set: stable sort + stable tiebreaker means the same input yields the same
 * colors across calls. Call this once per session (see map-page.ts) - running
 * it on a filtered subset would change which operators are "adjacent" and
 * could shift hues for operators that survived the filter.
 */
...
function chooseHue(
  ...
  for (const hue of unusedCandidates.length > 0 ? unusedCandidates : candidates) {
    ...
    // Adjacency separation is weighted 10x more than global separation; the
    // tiebreaker keeps the choice deterministic per operator id when scores
    // would otherwise tie.
